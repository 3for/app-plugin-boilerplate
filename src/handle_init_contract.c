...
// Called once to init.
void handle_init_contract(tronPluginInitContract_t *msg) {
    // Make sure we are running a compatible version.
    if (msg->interfaceVersion != TRON_PLUGIN_INTERFACE_VERSION_LATEST) {
        // If not the case, return the `UNAVAILABLE` status.
        msg->result = TRON_PLUGIN_RESULT_UNAVAILABLE;
        return;
    }
